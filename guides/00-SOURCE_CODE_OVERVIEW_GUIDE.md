...
`com.hirewise.be` (không phải đề xuất lý thuyết) + phần bổ sung mới
(mục 9 - Testing) do dự án hiện chưa có tài liệu riêng.
...
- **Luôn viết comment bằng tiếng Anh**, kể cả khi trao đổi trong team
  dùng tiếng Việt.
...
### 9.2. Cấu trúc thư mục & class test
...
### 9.3. Quy tắc đặt tên method test
...
**Quy tắc khi AI sinh test mới**: chọn 1 trong 2 dạng trên tuỳ độ dài
tình huống, LUÔN thể hiện rõ **điều kiện đầu vào** (state, role, dữ liệu
biên) và **kết quả mong đợi** (return giá trị gì / throw exception gì /
gọi hay không gọi 1 dependency nào đó) ngay trong tên method - không đặt
tên chung chung kiểu `test1`, `testCreate`, `testSuccess`.
...
### 9.4. Cấu trúc 1 test method - Arrange / Act / Assert

Giống coding convention ở mục 3 (cách dòng theo từng bước), mỗi test nên
tách rõ 3 khối bằng dòng trống: chuẩn bị dữ liệu/mock -> gọi hàm cần test
-> assert kết quả (+ verify tương tác với mock nếu cần):
...
### 9.5. Test level - dùng level nào cho layer nào
...
### 9.6. Checklist trước khi commit code có test
...
- [ ] Tên method test thể hiện rõ tình huống + kết quả mong đợi (mục
      9.3), không đặt tên chung chung.
